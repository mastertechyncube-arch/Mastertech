...
const DB_FILE = path.join(ROOT, 'data', 'mastertech-db.json');
  ...
  try{
    ...
    if(p.startsWith('/api/')){
      ...
      if(req.method==='POST'&&p==='/api/orders'){
        ...
        const order={orderId:'MT-'+Date.now().toString(36).toUpperCase()+'-'+Math.floor(Math.random()*900+100),date:new Date().toISOString(),status:'Pending',customer:b.customer,items:b.items,total:Number(b.total)||0,comment:String(b.comment||b.customer?.comment||''),liked:String(b.liked||b.customer?.liked||'No')};
      ...
      if(req.method==='GET'&&p==='/api/admin/dashboard')return json(res,200,{catalog:db.catalog,settings:db.settings,orders:db.orders,messages:db.messages,feedback:db.orders.filter(o=>o.comment||o.liked==='Yes')});
